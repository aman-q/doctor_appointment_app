...
function DoctorSuggestionList() {
    ...
    const getDoctorList = () => {
        GlobalApi.getDoctorList().then(resp => {
            ...
            const shuffledList = shuffleArray(allDoctors);
            const selectedDoctors = shuffledList.slice(0, 4); // Display only four doctors
            setDoctorList(selectedDoctors);
        });
    };
    ...
    return (
        <div className='p-4 border-[1px] mt-5 md:ml-5 rounded-lg'>
            ...
            {doctorList.map((doctor, index) => (
                <Link key={doctor.id} href={'/details/' + doctor.id} className='mb-4 p-3 shadow-sm w-full cursor-pointer hover:bg-slate-100 rounded-lg flex items-center gap-3'>
